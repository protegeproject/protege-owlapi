 ...
 * interpretation that has a set of anonymous and named individuals but all
 * class, object and data properties are empty.
 * 
 * @author tredmond
 */
public class TrivialModel {
    ...
    public TrivialModel(OWLOntology ontology, OWLReasonerFactory reasonerFactory)
            throws OWLOntologyCreationException {
        this.ontology = ontology;
        factory = ontology.getOWLOntologyManager()
                .getOWLDataFactory();
    ...
    private void initializeIndividuals() {
        ...
        allIndividuals.addAll(ontology
                .getReferencedAnonymousIndividuals(Imports.INCLUDED));
        if (allIndividuals.isEmpty()) {
            allIndividuals.add(Util.generateRandomEntity(factory,
                    EntityType.NAMED_INDIVIDUAL));
        }
    }

    private void initializeDataReasoner(OWLReasonerFactory reasonerFactory)
            throws OWLOntologyCreationException {
    ...
    public boolean isTopDataRange(OWLDataRange range) {
        return !reasoner.isSatisfiable(factory.getOWLObjectComplementOf(factory
                .getOWLDataAllValuesFrom(dp, range)));
    }

    public boolean isConsistent(OWLDataRange range) {
        return reasoner.isSatisfiable(factory.getOWLDataSomeValuesFrom(dp,
                range));
    }

    public boolean hasAtLeast(OWLDataRange range, int cardinality) {
        return reasoner.isSatisfiable(factory.getOWLDataMinCardinality(
                cardinality, dp, range));
    }

    public boolean hasNoMoreThan(OWLDataRange range, int cardinality) {
        return reasoner.isSatisfiable(factory.getOWLDataMaxCardinality(
                cardinality, dp, range));
    }

    public boolean hasExactly(OWLDataRange range, int cardinality) {
        return reasoner.isSatisfiable(factory.getOWLDataExactCardinality(
                cardinality, dp, range));
    }
}
